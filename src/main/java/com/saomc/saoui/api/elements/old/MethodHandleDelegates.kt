...
data class ImmutableFieldDelegate<in T, out V> @JvmOverloads constructor(val getter: (T) -> Any?, val cache: Boolean = false) :
...
data class MutableFieldDelegate<in T, V>(val getter: (T) -> Any?, val setter: (T, Any?) -> Unit) :
...
data class ImmutableStaticFieldDelegate<in T, out V> @JvmOverloads constructor(val getter: () -> Any?, val cache: Boolean = false) :
    ReadOnlyProperty<T, V> {
    private var cachedValue: V? = null
    private var initialized = false
...
data class MutableStaticFieldDelegate<in T, V>(val getter: () -> Any?, val setter: (Any?) -> Unit) :
    ReadWriteProperty<T, V> {
    @Suppress("UNCHECKED_CAST")

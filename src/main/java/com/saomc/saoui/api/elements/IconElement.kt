...
import com.saomc.saoui.util.getSystemTime
import com.teamwizardry.librarianlib.core.util.Client
import com.teamwizardry.librarianlib.core.util.DefaultRenderStates.TRANSLUCENT_TRANSPARENCY
...
//            SAOCore.LOGGER.info("Result: $field (tried $value)")
    ...
    override fun drawBackground(matrix: MatrixStack, mouse: Vec2d, partialTicks: Float) {
        ...
        controllingGUI?.blit(matrix, pos.xi, pos.yi, 1,  26, width, height)

        //GLCore.glTexturedRectV2(pos.x, pos.y, width = width.toFloat(), height = height.toFloat(), srcX = 1.0f, srcY = 26.0f)
    ...
    fun mouseOverEffect(){
        ...
        GLCore.glBlend(true)
        GLCore.blendFunc(GlStateManager.SourceFactor.SRC_COLOR.param, GlStateManager.DestFactor.ONE.param)
        ...
        GLCore.glTexturedRectV2(pos.x, pos.y, width = width.toFloat(), height = height.toFloat())
        GLCore.popMatrix()
        GLCore.pushMatrix()
        GLCore.scale(8.0f, 8.0f, 8.0f)
        ...
        GLCore.blendFunc(GlStateManager.SourceFactor.SRC_ALPHA.param, GlStateManager.DestFactor.ONE_MINUS_SRC_ALPHA.param)
    ...
    fun drawChildren(matrix: MatrixStack, mouse: Vec2d, partialTicks: Float, drawType: DrawType) {
        ...
        if (visibleElementsSequence.count() > 0) {
            ...
            childrenOrderedForRendering().forEachIndexed { i, it ->
                ...
                when (drawType){
                    DrawType.BACKGROUND -> it.drawBackground(matrix, nmouse, partialTicks)
                    DrawType.DRAW -> it.draw(matrix, nmouse, partialTicks)
                    DrawType.FOREGROUND -> it.drawForeground(matrix, nmouse, partialTicks)
                }
...
//        SAOCore.LOGGER.info("Showing $this")
    ...
    companion object {
        ...
        internal val BACKGROUND: RenderType by lazy {
            ...
            var state = RenderType.State.getBuilder()

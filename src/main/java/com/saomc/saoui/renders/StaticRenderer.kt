...
object StaticRenderer { // TODO: add usage of scale, offset etc from capability
    ...
    fun updateView(renderManager: EntityRendererManager){
        partialTicks = mc.renderPartialTicks
        for (i in 0..HEALTH_COUNT){
            val value = i.toDouble() / HEALTH_COUNT
            val rad = Math.toRadians((renderManager.info.pitch - 135).toDouble()) + (value - 0.5) * Math.PI * HEALTH_ANGLE
    ...
    private fun doRenderColorCursor(renderManager: EntityRendererManager, entity: LivingEntity, x: Double, y: Double, z: Double, distance: Int, color: ColorState) {

        if (entity.ridingEntity != null) return

        if (entity.world.isRemote) {
            val d3 = entity.getDistanceSq(renderManager.pointedEntity)

            if (d3 <= distance) {
                val sizeMult = if (entity.isChild && entity is MobEntity) 0.5f else 1.0f
                ...
                if (OptionCore.SPINNING_CRYSTALS.isEnabled) {
                    ...
                    if (a > Math.PI / 2 && a <= Math.PI * 3 / 2) {
                        GLCore.addVertex(9.0 * cos, -1.0, 9.0 * sin, 0.125f, 0.25f)
                        GLCore.addVertex(9.0 * cos, 17.0, 9.0 * sin, 0.125f, 0.375f)
                        GLCore.addVertex(-9.0 * cos, 17.0, -9.0 * sin, 0.0f, 0.375f)
                        GLCore.addVertex(-9.0 * cos, -1.0, -9.0 * sin, 0.0f, 0.25f)
                    } else {
                        GLCore.addVertex(-9.0 * cos, -1.0, -9.0 * sin, 0.0f, 0.25f)
                        GLCore.addVertex(-9.0 * cos, 17.0, -9.0 * sin, 0.0f, 0.375f)
                        GLCore.addVertex(9.0 * cos, 17.0, 9.0 * sin, 0.125f, 0.375f)
                    ...
                    if (a < Math.PI) {
                        GLCore.addVertex(-9.0 * sin, -1.0, 9.0 * cos, 0.125f, 0.25f)
                        GLCore.addVertex(-9.0 * sin, 17.0, 9.0 * cos, 0.125f, 0.375f)
                        GLCore.addVertex(9.0 * sin, 17.0, -9.0 * cos, 0.0f, 0.375f)
                        GLCore.addVertex(9.0 * sin, -1.0, -9.0 * cos, 0.0f, 0.25f)
                    } else {
                        GLCore.addVertex(9.0 * sin, -1.0, -9.0 * cos, 0.0f, 0.25f)
                        GLCore.addVertex(9.0 * sin, 17.0, -9.0 * cos, 0.0f, 0.375f)
                        GLCore.addVertex(-9.0 * sin, 17.0, 9.0 * cos, 0.125f, 0.375f)
                    ...
                    GLCore.addVertex(-9.0, -1.0, 0.0, 0.0f, 0.25f)
                    GLCore.addVertex(-9.0, 17.0, 0.0, 0.0f, 0.375f)
                    GLCore.addVertex(9.0, 17.0, 0.0, 0.125f, 0.375f)
                    GLCore.addVertex(9.0, -1.0, 0.0, 0.125f, 0.25f)
                }
                GLCore.draw()
    ...
    private fun doRenderHealthBar(renderManager: EntityRendererManager, living: LivingEntity
                                  , x: Double, y: Double, z: Double, distance: Int) {
        ...
        val d3 = living.getDistanceSq(renderManager.pointedEntity)
        ...
        if (d3 <= distance) {
            ...
            val size = if (living.isChild && living is MobEntity) 0.5f else 1.0f
            ...
            for (i in 0..hitPoints) {
                val value = (i + HEALTH_COUNT - hitPoints).toFloat() / HEALTH_COUNT
                val x0 = x + width * playerView[i + HEALTH_COUNT - hitPoints]!!.first
                val y0 = y + height
                val z0 = z + width * playerView[i + HEALTH_COUNT - hitPoints]!!.second

                val uv = value - (HEALTH_COUNT - hitPoints).toFloat() / HEALTH_COUNT

                GLCore.addVertex(x0, y0 + HEALTH_HEIGHT, z0, 1.0f - uv, 0.0f)
                GLCore.addVertex(x0, y0, z0, 1.0f - uv, 0.125f)
            }
            ...
            for (i in 0..HEALTH_COUNT) {
                val value = i.toFloat() / HEALTH_COUNT

                val x0 = x + width * playerView[i]!!.first
                val y0 = y + height
                val z0 = z + width * playerView[i]!!.second

                GLCore.addVertex(x0, y0 + HEALTH_HEIGHT, z0, 1.0f - value, 0.125f)
                GLCore.addVertex(x0, y0, z0, 1.0f - value, 0.25f)
            }
        ...
        if (OptionCore.PARTICLES.isEnabled) {
            ...
            if (living.world.isRemote) {
                ...
                for (i in 0 until pieces) {
                    ...
                    mc.world.addParticle(DeathParticles(
                            world,

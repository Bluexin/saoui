...
private class FieldListItem(val target: Class<*>, val name: String) {
    ...
    fun createRootGetter(): (target: Any) -> Any? {
        ...
        when (accessorOfChoice) {
            ...
            is Int -> getter = { t -> ArrayReflect[t, accessorOfChoice] }
            else -> throw IllegalStateException(
                "accessorOfChoice was neither a Field nor a KProperty, it was `${(accessorOfChoice as Any?)?.javaClass?.canonicalName
                    ?: "null"}`"
            )
        }
    ...
    fun createRootSetter(): (target: Any, finalValue: Any?) -> Any? {
        ...
        val setter by lazy<(target: Any, value: Any?) -> Any?> {
            if (accessorOfChoice is Int) {
                return@lazy m@{ t, v ->
                    ArrayReflect[t, accessorOfChoice] = v
            ...
            when (accessorOfChoice) {
                ...
                is KMutableProperty<*> -> {
                    ...
                    throw IllegalStateException(
                        "accessorOfChoice was neither a Field nor a KProperty, it was `${(
                            accessorOfChoice as Any?
                            )?.javaClass?.canonicalName ?: "null"}`"
                    )
            }
        } else {

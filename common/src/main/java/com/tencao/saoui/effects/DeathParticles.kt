...
class DeathParticles private constructor(world: ClientLevel, xCoord: Double, yCoord: Double, zCoord: Double, redValue: Float, greenValue: Float, blueVale: Float, scale: Float) : Particle(world, xCoord, yCoord, zCoord, 0.0, 0.0, 0.0) {
    ...
    constructor(world: ClientLevel, xCoord: Double, yCoord: Double, zCoord: Double, redValue: Float, greenValue: Float, blueValue: Float) : this(world, xCoord, yCoord, zCoord, redValue, greenValue, blueValue, 1.0f)

    init {
        this.xd = ((Math.random() * 2.0 - 1.0).toFloat() * 0.05f).toDouble()
        this.yd = ((Math.random() * 2.0 - 1.0).toFloat() * 0.05f).toDouble()
        this.zd = ((Math.random() * 2.0 - 1.0).toFloat() * 0.05f).toDouble()
        this.rotationY = random.nextFloat() * 2
        this.speedRotationY = (random.nextFloat() + 2.0f) / if (random.nextBoolean()) 16.0f else -16.0f
        this.rCol = redValue
        this.gCol = greenValue
        this.bCol = blueVale
        scale(scale)
        this.lifetime = (8.0 / (Math.random() * 0.8 + 0.2)).toInt()
        this.lifetime = (this.lifetime.toFloat() * scale).toInt()
    }

    override fun render(buffer: VertexConsumer, renderInfo: Camera, partialTicks: Float) {
        this.time = partialTicks
        this.particleX = renderInfo.blockPosition.x.toFloat()
        this.particleY = renderInfo.blockPosition.y.toFloat()
        this.particleZ = renderInfo.blockPosition.z.toFloat()
        this.f0 = renderInfo.xRot
        this.f1 = renderInfo.yRot

        queuedRenders.add(this)

        var particle = (this.age.toFloat() + time) / this.lifetime.toFloat() * 32.0f

        if (particle < 0.0f) particle = 0.0f

        if (particle > 1.0f) particle = 1.0f

        val particleWidth = 0.1f * this.bbWidth * particle
        val particleHeight = 0.1f * this.bbHeight * particle
        val xPos = (this.xo + (this.x - this.xo) * time).toFloat()
        val yPos = (this.yo + (this.y - this.yo) * time).toFloat()
        val zPos = (this.zo + (this.z - this.zo) * time).toFloat()
        ...
        if (a < Math.PI) {
            buffer.vertex(xPos + x1 * cos, yPos + y1, zPos + z1 * sin).uv(0.0f, 1.0f).color(this.rCol * colorIntensity, this.gCol * colorIntensity, this.bCol * colorIntensity, 1f).endVertex()
            buffer.vertex(xPos + x1 * cos, yPos + y1, zPos + z1 * sin).uv(0.0f, 1.0f).color(this.rCol * colorIntensity, this.gCol * colorIntensity, this.bCol * colorIntensity, 1f).endVertex()
            buffer.vertex(xPos + x2 * cos, yPos + y2, zPos + z2 * sin).uv(1.0f, 1.0f).color(this.rCol * colorIntensity, this.gCol * colorIntensity, this.bCol * colorIntensity, 1f).endVertex()
            buffer.vertex(xPos + x3 * cos, yPos + y3, zPos + z3 * sin).uv(1.0f, 0.0f).color(this.rCol * colorIntensity, this.gCol * colorIntensity, this.bCol * colorIntensity, 1f).endVertex()
            buffer.vertex(xPos + x4 * cos, yPos + y4, zPos + z4 * sin).uv(0.0f, 0.0f).color(this.rCol * colorIntensity, this.gCol * colorIntensity, this.bCol * colorIntensity, 1f).endVertex()
        } else {
            buffer.vertex(xPos - x1 * cos, yPos + y1, zPos - z1 * sin).uv(0.0f, 1.0f).color(this.rCol * colorIntensity, this.gCol * colorIntensity, this.bCol * colorIntensity, 1f).endVertex()
            buffer.vertex(xPos - x2 * cos, yPos + y2, zPos - z2 * sin).uv(1.0f, 1.0f).color(this.rCol * colorIntensity, this.gCol * colorIntensity, this.bCol * colorIntensity, 1f).endVertex()
            buffer.vertex(xPos - x3 * cos, yPos + y3, zPos - z3 * sin).uv(1.0f, 0.0f).color(this.rCol * colorIntensity, this.gCol * colorIntensity, this.bCol * colorIntensity, 1f).endVertex()
    ...
    override fun renderParticle(worldrender: BufferBuilder, entity: Entity?, time: Float, x: Float, y: Float, z: Float, f0: Float, f1: Float) {
        ...
        if (a < Math.PI) {
            worldrender.pos(xPos + x1 * cos, yPos + y1, zPos + z1 * sin).tex(0.0f, 1.0f).color(this.rCol * colorIntensity, this.gCol * colorIntensity, this.bCol * colorIntensity, 1f).endVertex()
            worldrender.pos(xPos + x1 * cos, yPos + y1, zPos + z1 * sin).tex(0.0f, 1.0f).color(this.rCol * colorIntensity, this.gCol * colorIntensity, this.bCol * colorIntensity, 1f).endVertex()
            worldrender.pos(xPos + x2 * cos, yPos + y2, zPos + z2 * sin).tex(1.0f, 1.0f).color(this.rCol * colorIntensity, this.gCol * colorIntensity, this.bCol * colorIntensity, 1f).endVertex()
            worldrender.pos(xPos + x3 * cos, yPos + y3, zPos + z3 * sin).tex(1.0f, 0.0f).color(this.rCol * colorIntensity, this.gCol * colorIntensity, this.bCol * colorIntensity, 1f).endVertex()
            worldrender.pos(xPos + x4 * cos, yPos + y4, zPos + z4 * sin).tex(0.0f, 0.0f).color(this.rCol * colorIntensity, this.gCol * colorIntensity, this.bCol * colorIntensity, 1f).endVertex()
        } else {
            worldrender.pos(xPos - x1 * cos, yPos + y1, zPos - z1 * sin).tex(0.0f, 1.0f).color(this.rCol * colorIntensity, this.gCol * colorIntensity, this.bCol * colorIntensity, 1f).endVertex()
            worldrender.pos(xPos - x2 * cos, yPos + y2, zPos - z2 * sin).tex(1.0f, 1.0f).color(this.rCol * colorIntensity, this.gCol * colorIntensity, this.bCol * colorIntensity, 1f).endVertex()
            worldrender.pos(xPos - x3 * cos, yPos + y3, zPos - z3 * sin).tex(1.0f, 0.0f).color(this.rCol * colorIntensity, this.gCol * colorIntensity, this.bCol * colorIntensity, 1f).endVertex()

...
object StaticPlayerHelper {
    ...
    fun listOnlinePlayers(mc: Minecraft, range: Double): List<Player> {
        return mc.level?.players()?.filter { p -> mc.player!!.distanceTo(p) <= range } ?: emptyList()
    }

    private fun listOnlinePlayers(mc: Minecraft): List<Player> {
        return mc.level?.players() ?: emptyList()
    }

    fun findOnlinePlayer(mc: Minecraft, username: String): Player? {
        return mc.level?.players()?.firstOrNull { it.displayName.contents.equals(username, true) }
    }
    ...
    fun getName(player: Player?): String {
        return if (player == null) "" else player.displayName.contents
    }

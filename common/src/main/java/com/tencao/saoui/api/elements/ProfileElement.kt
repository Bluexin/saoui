...
class ProfileElement(var player: Player? = Client.player) : IconElement(IconCore.PROFILE) {

    override var listed: Boolean = player != Client.player
    var w = 165
    var h = 256
    val size = 40
    ...
    private val rl = ResourceLocation(Constants.MODID, "textures/menu/parts/profilebg.png")
    ...
    init {
        if (player == null) {
            ...
            player?.armorSlots?.forEachIndexed { index, itemStack ->
    ...
    override fun drawBackground(mouse: Vector2d, partialTicks: Float, poseStack: PoseStack) {
        if (canDraw) {
            ...
            GLCore.glString(player!!.scoreboardName, pos.xi + 50 + (player!!.displayName.string.length / 2), pos.yi + 20, ColorUtil.DEFAULT_BOX_FONT_COLOR.rgba, poseStack = poseStack, shadow = false, centered = true)
            ...
            profile.indices.forEach {
                GLCore.glString(profile[it], pos.xi + (w / 2) - 10 + (-GLCore.glStringWidth(profile[it]) / 2), (pos.y + 180 + (it * GLCore.glStringHeight())).toInt(), color, poseStack = poseStack, centered = false)
            }
    ...
    private fun drawCharacter(x: Double, y: Double) {
        ...
        GLCore.depth(true)
        if (tmp != null && OptionCore.MOUNT_STAT_VIEW.isEnabled) {
            InventoryScreen.renderEntityInInventory(x.roundToInt(), y.roundToInt(), 40, Client.minecraft.screen!!.width / 3.5f, 20f, tmp)
        } else {
            InventoryScreen.renderEntityInInventory(x.roundToInt(), y.roundToInt(), 40, Client.minecraft.screen!!.width / 3.5f, 20f, player)
        }

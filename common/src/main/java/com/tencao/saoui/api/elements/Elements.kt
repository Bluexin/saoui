...
interface INeoParent {
    ...
    fun move(delta: Vector2d) {
        try {
            ...
            Constants.LOGGER.error("Element caused a concurrent modification exception on animation")
            e.printStackTrace()
        }
        destination += delta
...
interface NeoElement : INeoParent {
    ...
    var opacity
        ...
        set(_) = Unit
    val scale
        get() = Vector2d(1.0, 1.0)
    val canDraw: Boolean
        get() = opacity >= 0.03 || scale != Vector2d(0, 0)

    fun drawBackground(mouse: Vector2d, partialTicks: Float, poseStack: PoseStack)
    fun draw(mouse: Vector2d, partialTicks: Float, poseStack: PoseStack)
    fun drawForeground(mouse: Vector2d, partialTicks: Float, poseStack: PoseStack)

...
//        if (OptionCore.CUSTOM_FONT.isEnabled) GLCore.setFont(Minecraft.getMinecraft(), OptionCore.CUSTOM_FONT.isEnabled)
        ...
        runCatching {
            ...
            Constants.LOGGER.warn("Failed to load $theme", it)
...
//        StringNames.init()
    ...
    private fun loadCss(location: ResourceLocation) {
        ...
        Client.resourceManager.getResource(location).let { styleSheet ->
            try {
                val aCSS = CSSReader.readFromStream(
                    object : IHasInputStream {
                        override fun isReadMultiple() = false
                        override fun getInputStream() = styleSheet.inputStream
                    },
        ...
        Constants.LOGGER.info("Loaded CSS in " + (System.currentTimeMillis() - start) + "ms.")
    }
}
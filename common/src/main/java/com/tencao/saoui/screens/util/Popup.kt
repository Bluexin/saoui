...
open class Popup<T : Any>(open var header: String, open var text: List<String>, open var footer: String, val map: Map<IconElement, T>) : CoreGUI<T>(header, Vector2d(0, 0)) {

    private val rl = ResourceLocation(MODID, "textures/menu/parts/alertbg.png")
...
//        if (shadows < 20.0) SAOCore.LOGGER.info("h=$h; shadows=$shadows; step3=$step3; expansion=$expansion")
        ...
        val posX = -w / 2.0
        val posY = -h / 2.0
        GLCore.glTexturedRectV2(posX, posY, width = w.toDouble(), height = step1, srcX = 0.0, srcY = 0.0, srcWidth = 256.0, srcHeight = 64.0) // Title bar
        GLCore.glTexturedRectV2(posX, posY + step1, width = w.toDouble(), height = shadows, srcX = 0.0, srcY = 64.0, srcWidth = 256.0, srcHeight = 32.0) // Top shadow
        GLCore.glTexturedRectV2(posX, posY + step1 + shadows, width = w.toDouble(), height = step3, srcX = 0.0, srcY = 96.0, srcWidth = 256.0, srcHeight = 32.0) // Text lines
        GLCore.glTexturedRectV2(posX, posY + step1 + shadows + step3, width = w.toDouble(), height = shadows, srcX = 0.0, srcY = 128.0, srcWidth = 256.0, srcHeight = 32.0) // Bottom shadow
        GLCore.glTexturedRectV2(posX, posY + step1 + shadows + step3 + shadows, width = w.toDouble(), height = step5, srcX = 0.0, srcY = 160.0, srcWidth = 256.0, srcHeight = 96.0) // Button bar

        if (alpha > 0.03f) GLCore.glString(header, -GLCore.glStringWidth(header) / 2, (-h / 2.0 + step1 / 2).toInt(), ColorUtil.DEFAULT_BOX_FONT_COLOR.multiplyAlpha(alpha), poseStack = poseStack, centered = true)
        (text.indices).forEach {
            if (alpha > 0.56f) GLCore.glString(text[it], -GLCore.glStringWidth(text[it]) / 2, (-h / 2.0 + step1 + shadows + step3 / (text.size) * (it + 0.5)).toInt(), ColorUtil.DEFAULT_FONT_COLOR.multiplyAlpha((alpha - 0.5f) / 0.5f), poseStack = poseStack, centered = true)
        }
        if (alpha > 0.03f) GLCore.glString(footer, -GLCore.glStringWidth(footer) / 2, (-h / 2.0 + step1 + shadows + step3 + (step5 / 2)).toInt(), ColorUtil.DEFAULT_BOX_FONT_COLOR.multiplyAlpha(alpha), poseStack = poseStack, centered = true)
...
class PopupSlotSelection(header: String, text: List<String>, footer: String, slots: Set<Slot>) : Popup<Int>(header, text, footer, getButtons(slots)) {
    ...
    companion object {
        ...
        fun getButtons(slots: Set<Slot>): Map<IconElement, Int> {
            ...
            slots.forEach {
                map[IconElement(it.item.toIcon())] = it.index
            }
            return map
        }
    }
...
class PopupHotbarSelection(header: String, text: List<String>, footer: String) : Popup<PopupHotbarSelection.Result>(header, text, footer, getHotbarList()) {
    ...
    companion object {
        ...
        fun getHotbarList(): Map<IconElement, Result> {
            ...
            val inventory = Client.player!!.containerMenu
            for (i in 36..44) {
                val stack = inventory.getSlot(i).item
                map[
...
class PopupPlayerInspect(player: Player, elements: List<IconElement>) : Popup<Int>(
    player.displayName.contents,
    player.let {
        PlayerStats.instance().stats.getStatsString(it)
    },
    "",
...
class PopupAdvancement(advancement: Advancement) : Popup<PopupAdvancement.Result>(
    advancement.chatComponent.contents,

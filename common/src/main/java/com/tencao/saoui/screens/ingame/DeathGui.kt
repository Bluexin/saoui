...
class DeathGui : Screen("Death Screen".toTextComponent()) {
    ...
    private var isHardCore = Client.minecraft.level?.levelData?.isHardcore ?: false
    var counter = 0
    ...
    fun darkenScreen(poseStack: PoseStack) {
        val alpha = counter / 40f
        fill(poseStack, 0, 0, scaledWidth, scaledHeight, ColorUtil.multiplyAlpha(Color.black.rgb, alpha))
/*
    ...
    companion object {
        val rl = ResourceLocation(MODID, "textures/hud/buttons/death.png")
        const val w = 280f
        const val h = 100f
    }
}

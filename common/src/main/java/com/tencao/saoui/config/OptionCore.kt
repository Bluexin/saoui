...
enum class OptionCore(
    ...
    override val category: OptionCategory
    ...
    val displayName: Component = unformattedName.translate()
    val description: List<String> = listOf("$unformattedName.desc".localize())

    /**
     * This is set when the config is loaded and will store the current value
     */
    var value by BooleanSetting(Settings.NS_BUILTIN, ResourceLocation("options", unformattedName), defaultValue, description.joinToString()).register("general")

...
object Settings {
    val NS_BUILTIN = ResourceLocation(Constants.MODID, "builtin")

    private val logger = LogManager.getLogger(this)

    private var configBuilding: MutableMap<String, ForgeConfigSpec.Builder> = mutableMapOf()
    private var configBuilt: MutableMap<String, ModConfig> = mutableMapOf()
    ...
    fun register(setting: Setting<*>, config: String) {
        if (configBuilt[config] != null) throw IllegalStateException("Tried to register config after been built")
    ...
    private fun registerNamespace(namespace: ResourceLocation, config: String) {
        if (namespace !in configurations) {
            ...
            dir.mkdirs()
            // TODO
            configurations[namespace] = ConfigurationData(namespace)
    ...
    fun buildConfig(config: String){
        configBuilding[config]?.let {
            configBuilding.remove(config)
            configBuilt[config] = registerConfig(it)
        }
    }

    fun registerConfig(spec: ForgeConfigSpec.Builder): ModConfig {
        return ModLoadingContext.registerConfig(Constants.MODID, ModConfig.Type.CLIENT, spec.build())
    }

    ...
    while(true) {
      ...
      default:
        ...
        if (value<=127) val=new Byte((byte)value);
                 ...
                 (value<=4294967295L)) // we allow HEX and octal ints
...
//      } 
      ...
      if ((typeID==8) &&
          (
           (clazz=(lib.cnmap==null?null:
                   lib.cnmap.get(typeAccum.substring(accumStrt))
                   )
            )==null
           )
          )
  ...
  private final void genDVCall()
    ...
    if (lib.resolver instanceof DVMap)
      varObj=lib.resolver.translate(varName);
    else
      varObj=varName;
  ...
  private final void functionCall(Class<?> resolveInClass,String name,
    ...
    paramOPs.push(new OPcall(m, np, paramOPs, lib.isStateless(m)));
  }

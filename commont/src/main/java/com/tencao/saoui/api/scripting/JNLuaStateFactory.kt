...
abstract class JNLuaStateFactory {
    ...
    fun init() {
        ...
        if (SystemUtils.IS_OS_WINDOWS && !ConfigHandler.alwaysTryNative) {
            if (SystemUtils.IS_OS_WINDOWS_XP) {
                Constants.LOGGER.warn("Sorry, but Windows XP isn't supported. I'm afraid you'll have to use a newer Windows. I very much recommend upgrading your Windows, anyway, since Microsoft has stopped supporting Windows XP in April 2014.")
                return
            }

            if (SystemUtils.IS_OS_WINDOWS_2003) {
                Constants.LOGGER.warn("Sorry, but Windows Server 2003 isn't supported. I'm afraid you'll have to use a newer Windows.")
                return
            }
        }
        ...
        if (!Strings.isNullOrEmpty(ConfigHandler.forceNativeLibPathFirst)) {
            ...
            if (libraryTest.canRead()) {
                ...
                Constants.LOGGER.info("Found forced-path filesystem library $currentLib.")
            } else
                Constants.LOGGER.warn("forceNativeLibPathFirst is set, but $currentLib was not found there. Falling back to checking the built-in libraries.")
        }

        if (currentLib.isEmpty()) {
            val libraryUrl = javaClass.getResource("/assets/${Constants.MODID}/lib/$libraryName")
            if (libraryUrl == null) {
                Constants.LOGGER.warn("Native library with name '$libraryName' not found.")
                return
            }

            val tmpLibName = "${Constants.MODID}-${Constants.MOD_VERSION}-$version-$libraryName"
                ...
                if (libDir.isDirectory) {
                    for (file in libDir.listFiles(
                        PatternFilenameFilter(
                            "^" + Pattern.quote("${Constants.MODID}-") + ".*" + Pattern.quote(
                                "-$libraryName"
                            ) + "$"
                        )
                    )!!) {
            ...
            if (tmpLibFile.exists()) {
                ...
                if (!matching) {
                    ...
                    if (tmpLibFile.exists()) {
                        Constants.LOGGER.warn("Could not update native library '${tmpLibFile.name}'!")
                    }
                }
            }
        ...
        try {
            ...
            Constants.LOGGER.info("Found a compatible native library: '${tmpLibFile.name}'.")
            haveNativeLibrary = true
        } catch (t: Throwable) {
            if (ConfigHandler.logFullLibLoadErrors) {
                Constants.LOGGER.warn("Could not load native library '${tmpLibFile.name}'.", t)
            } else {
                Constants.LOGGER.trace("Could not load native library '${tmpLibFile.name}'.")
            }
            tmpLibFile.delete()
        }
    }
    ...
    init {
        ...
        if (!haveNativeLibrary) {
            Constants.LOGGER.warn("Platform doesn't support JNLua !")
        }
    }
    ...
    fun createState(): LuaState? {
        ...
        try {
            ...
            try {
                ...
                Constants.LOGGER.warn("Failed creating Lua state.", t)
                state.close()
            }
        } catch (_: UnsatisfiedLinkError) {
            Constants.LOGGER.error("Failed loading the native libraries.")
        } catch (t: Throwable) {
            Constants.LOGGER.warn("Failed creating Lua state.", t)

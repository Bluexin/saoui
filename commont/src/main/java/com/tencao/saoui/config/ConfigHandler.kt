...
object ConfigHandler{
    ...
    var lastVersion by StringSetting(NS_BUILTIN, general("last_update"), "nothing").register("general")
    var ignoreUpdate by BooleanSetting(NS_BUILTIN, general("ignore_update"), true).register("general")
    var enableDebug by BooleanSetting(NS_BUILTIN, general("debug"), false).register("general")
    var debugFakePT by IntSetting(
        NS_BUILTIN, general("debug_fake_pt"), 0, "Amount of fake party members, 0 to disable."
    ) { it in 0..10 }.register("general")
    ...
    fun loadConfig(modConfig: ModConfig){
    }

...
object EventCore {
    ...
    fun onDeath(e: LivingDeathEvent) {
        if (e.entityLiving != null && e.entityLiving.level.isClientSide && e.entityLiving.isDeadOrDying) {
    ...
    fun renderEntityListener(e: RenderLivingEvent.Pre<*, *>) {
        if (e.entity.distanceToSqr(mc.player as ClientPlayerEntity) > (mc.options.renderDistance * 16).toDouble().pow(2.0)) {
            e.isCanceled = true
        }
    }
    ...
    fun partyInviteCancelled(e: PartyEvent.InviteCanceled) {
        ...
        if (notification != null) {
            ...
            (mc.screen as? CoreGUI<*>)?.getPopup?.text?.any { line ->
                line.contains(e.partyData.leaderInfo.username, true)
            }?.run {
                (mc.screen as? CoreGUI<*>)?.getPopup?.onClose()
            }
        }
    }
    ...
    fun partyInvite(e: PartyEvent.Invited) {
        ...
        if (e.player == player) {
            ...
            if (mc.screen is CoreGUI<*>) {
                (mc.screen as CoreGUI<*>).openGui(
                    partyNotification
                )
            } else {
    ...
    internal val player
        get() = PlayerInfo(mc.user.gameProfile)
}

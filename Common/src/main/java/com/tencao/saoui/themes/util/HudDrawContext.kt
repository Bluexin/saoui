 ...
 * @author Bluexin
 */
open class HudDrawContext(player: PlayerEntity = Client.player!!, val mc: Minecraft = Client.minecraft, itemRenderer: ItemRenderer = Client.minecraft.itemRenderer) :
    IHudDrawContext {
    /*
    Feel free to add anything you'd need here.
     */
    private val username: String = player.displayName.string
    override val fontRenderer: FontRenderer = GLCore.glFont
    override val itemRenderer: ItemRenderer
    private val usernameWidth: Double = (1 + (mc.font.width(username) + 4) / 5.0) * 5
    private val stats: PlayerStatsProvider = instance().stats
    override lateinit var player: PlayerEntity
    private var healthStep: HealthStep? = null
    override var z = 0f
    private var hp = 0f
    private var maxHp = 0f
    private lateinit var scaledResolution: MainWindow
    ...
    fun setScaledResolution(scaledResolution: MainWindow) {
    ...
    override fun horsejump(): Float {
        return (player as ClientPlayerEntity).jumpRidingScale
    }
    ...
    override fun entityName(index: Int): String = nearbyEntities[index].get()?.displayName?.string?: "Unknown"

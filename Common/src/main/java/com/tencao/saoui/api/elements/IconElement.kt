...
import com.tencao.saomclib.scaledHeight
import com.tencao.saomclib.scaledWidth
...
//            Constants.LOG.info("Result: $field (tried $value)")
    ...
    override fun drawForeground(mouse: Vec2d, partialTicks: Float, matrixStack: MatrixStack) {
        ...
        GLCore.pushMatrix()
        if (mouse in this) {
            drawHoveringText(matrixStack, mouse)
    ...
    open fun drawHoveringText(matrixStack: MatrixStack, mouse: Vec2d) {
        GuiUtils.drawHoveringText(
            matrixStack,
            description.map { it.toTextComponent() },
            mouse.xi,
            mouse.yi,
            Client.minecraft.scaledWidth,
            Client.minecraft.scaledHeight,
            width - 20,
            GLCore.glFont
        )

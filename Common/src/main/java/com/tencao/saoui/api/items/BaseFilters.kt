...
enum class BaseFilters(val filter: (ItemStack, Boolean) -> Boolean) : IItemFilter { // Todo: support for TConstruct
    ...
    HELMET({ stack, _ ->
        Client.player!!.containerMenu.slots.first { it.slotIndex == 5 }.mayPlace(stack)
    }) {
        override val icon: IIcon
    ...
    CHESTPLATES({ stack, _ ->
        Client.player!!.containerMenu.slots.first { it.slotIndex == 6 }.mayPlace(stack)
    }) {
        override val icon: IIcon
    ...
    LEGGINS({ stack, _ ->
        Client.player!!.inventoryMenu.slots.first { it.slotIndex == 7 }.mayPlace(stack)
    }) {
        override val icon: IIcon
    ...
    BOOTS({ stack, _ ->
        Client.player!!.inventoryMenu.slots.first { it.slotIndex == 8 }.mayPlace(stack)
    }) {
        override val icon: IIcon
    ...
    SWORDS({ stack, _ -> stack.item is SwordItem || stack.toolTypes.contains(ToolType.get("SWORD")) }) {
        override val icon: IIcon
    ...
    BOWS({ stack, _ -> stack.item is BowItem || stack.item.getUseAnimation(stack) == UseAction.BOW || stack.toolTypes.contains(ToolType.get("BOW")) }) {
        override val icon: IIcon
    ...
    PICKAXES({ stack, _ -> stack.item is PickaxeItem || stack.toolTypes.contains(ToolType.PICKAXE) }) {
        override val icon: IIcon
    ...
    AXES({ stack, _ -> stack.item is AxeItem || stack.toolTypes.contains(ToolType.AXE) }) {
        override val icon: IIcon

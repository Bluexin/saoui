 ...
 */
class BoundPropertyDelegateReadOnly<in R, T>(private val prop: KProperty0<T>) : ReadOnlyProperty<R, T> {
    override fun getValue(thisRef: R, property: KProperty<*>): T {
        return prop.get()
    }
}

class BoundPropertyDelegateReadWrite<in R, T>(private val prop: KMutableProperty0<T>) : ReadWriteProperty<R, T> {
    override fun getValue(thisRef: R, property: KProperty<*>): T {
        return prop.get()
    }

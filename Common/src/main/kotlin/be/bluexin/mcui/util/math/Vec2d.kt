...
@Deprecated("in favor of joml", replaceWith = ReplaceWith("Vector2d", "org.joml.Vector2d"))
class Vec2d(val x: Double, val y: Double) {
    ...
    fun floor(): Vec2d {
        return getPooled(Math.floor(x), Math.floor(y))
    }

    fun ceil(): Vec2d {
        return getPooled(Math.ceil(x), Math.ceil(y))
    }
    ...
    fun setX(value: Double): Vec2d {
        return getPooled(value, y)
    }

    fun setY(value: Double): Vec2d {
        return getPooled(x, value)
    }

    fun setAxis(axis: Axis2d, value: Double): Vec2d {
        return when (axis) {
            Axis2d.X -> getPooled(value, y)
            Axis2d.Y -> getPooled(x, value)
        }
    }
    ...
    @Deprecated(
        message = "Deprecated for boxing reasons. Use the primitive version instead.",
    ...
    fun toFloat(): Vector2f = Vector2f(xf, yf)
    ...
    companion object {
        ...
        val INFINITY = Vec2d(Double.POSITIVE_INFINITY)

        @JvmField
        @Deprecated("These are stupid. Past me was stupid.")
        val NEG_INFINITY = Vec2d(Double.NEGATIVE_INFINITY)
        ...
        fun min(a: Vec2d, b: Vec2d): Vec2d {
            return getPooled(a.x.coerceAtMost(b.x), a.y.coerceAtMost(b.y))
        }
        ...
        fun max(a: Vec2d, b: Vec2d): Vec2d {
            return getPooled(a.x.coerceAtLeast(b.x), a.y.coerceAtLeast(b.y))
        }
        ...
        @JvmStatic
        fun getPooled(d: Double): Vec2d = getPooled(d, d)
